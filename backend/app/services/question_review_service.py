...
async def ensure_question_reviews(
    ...
    """Create one stable review snapshot per score and repair incomplete legacy rows."""
    await db.execute(
...
def build_question_review_payload(score: InterviewScore) -> dict[str, Any]:
    """Build a complete, fact-grounded snapshot from a saved score."""
...
def build_grounded_sample_answer(original_answer: str, suggestions: list[str]) -> str:
    """Return a safe template that never invents candidate companies, metrics or experience."""
