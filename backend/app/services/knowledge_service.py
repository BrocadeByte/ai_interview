...
async def _load_document_content(document: KnowledgeDocument) -> str:
    ...
    try:
        ...
        # Uploaded documents keep an 8,000-character database copy. It is a valid recovery
        # source when object storage is temporarily unavailable, and is revalidated below.
        if document.content:
            logger.warning(
...
async def _load_validated_document_payload(document: KnowledgeDocument) -> KnowledgeDocumentCreate:
    """Validate persisted text and, when possible, repair it by reparsing the original object."""
...
async def update_knowledge_document(
    ...
    """Replace one document version without rebuilding or hiding unrelated documents."""
    document = await db.scalar(select(KnowledgeDocument).where(KnowledgeDocument.id == document_id))
    if not document:
        return None
    try:
...
async def delete_knowledge_document(db: AsyncSession, document_id: int) -> bool:
    """Delete only one document's points; the active collection remains online."""
    document = await db.scalar(select(KnowledgeDocument).where(KnowledgeDocument.id == document_id))
    if not document:
        return False
    try:
...
async def reindex_knowledge_documents(db: AsyncSession) -> KnowledgeReindexResult:
    """Build a new physical collection and atomically switch the active alias."""
...
def unpack_knowledge_context(value: str | KnowledgeContextResult | dict[str, Any]) -> tuple[str, list[dict[str, Any]]]:
    """Normalize structured retrieval results while keeping string-returning test doubles compatible."""
