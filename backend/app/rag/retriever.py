...
async def ensure_collection() -> None:
    """Ensure either the active alias or the legacy collection is available."""
...
async def switch_active_collection(target_collection: str) -> str:
    """Atomically point the stable active alias at a fully built collection."""
    alias = active_alias_name()
...
async def upsert_chunks(chunks: list[dict], *, collection_name: str | None = None) -> None:
    """Persist chunks to Qdrant with stable IDs and searchable scope metadata."""
    if not chunks:
        return
...
async def replace_document_chunks(document_id: int, index_version: int, chunks: list[dict]) -> None:
    """Publish a document version, then remove only stale versions of that document."""
    await ensure_collection()
    collection_name = await get_active_collection_name()
...
async def search_chunks_advanced(
    ...
    *,
    ...
    """Run scoped Dense + BM25 recall, model rerank, adjacent expansion and context budgeting."""
    if not query:
        return []
...
def _legacy_title_matches(chunk: dict, target_position: str) -> bool:
    """Scope legacy general/file-category documents by a position signal in their title."""
...
def _bm25_recall(query: str, candidates: list[dict], limit: int) -> list[dict]:
    """Perform real BM25 scoring over the persisted, scope-filtered candidate set."""
    terms = _query_terms(query)
...
async def _expand_adjacent_chunks(chunks: list[dict]) -> list[dict]:
    """Attach immediate same-document neighbors for boundary context."""
...
def _apply_context_token_budget(chunks: list[dict], budget: int) -> list[dict]:
    """Use one predictable budget for every Agent knowledge-context call."""
    if budget <= 0:
        return []
    remaining = budget
    selected: list[dict] = []
